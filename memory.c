...
void addObjHeaderToList(ObjHeader* Header)
{
...
void scanner()
{
	...
	while(tmpHead){
		void* Header = (void*)tmpHead -> Header;
		scanRoots((unsigned*)(Header + OBJ_HEADER_SIZE), (unsigned*)(Header + tmpHead -> Header -> Size));
		Node* n = tmpHead -> Next;
		free(tmpHead);
		tmpHead = n;
	}
...
void sweep()
{
	...
	while(SegList){
		...
		char *start = getDataPtr(Seg);
		char *end = getAllocPtr(Seg);
		while(start < end){
			if (getSizeMetadata(ADDR_TO_PAGE(start))[0] < PAGE_SIZE){
				ObjHeader *Header = (ObjHeader*)start;
				start += Header -> Size;
				if(Header -> Status != MARK && Header -> Status != FREE){
					myfree((char*)Header + OBJ_HEADER_SIZE);
				}
				else if(Header -> Status == MARK)
					Header -> Status = 0;
			}
			else{
				start = ADDR_TO_PAGE(start) + PAGE_SIZE;
			}
 ...
 * for scanning.
 */
int inSegmentList(void *Add){
	SegmentList *SegList = Segments;
	while(SegList){
		if(ADDR_TO_SEGMENT(Add) == SegList -> Segment)
			return 1;
		SegList = SegList -> Next; 
	}
	return 0;
...
ObjHeader* getObjHeader(char *add){
	Segment *Seg = ADDR_TO_SEGMENT(add);
	int isBig = getBigAlloc(Seg);
	if(isBig){
		while(getSizeMetadata(add)[0] != 1){
			add -= PAGE_SIZE;
		}
		return (ObjHeader*)ADDR_TO_PAGE(add);
	}
	else{
		// page start always contains a object header
		char *Page = ADDR_TO_PAGE(add);
		ObjHeader* Header = (ObjHeader*)Page;
		while(((char*)Header + OBJ_HEADER_SIZE) >= add && add < ((char*)Header + Header -> Size)){
			Header = (ObjHeader*)((char*)Header + Header -> Size);
		}
		return Header;
	}
	return NULL;
...
static void scanRoots(unsigned *Top, unsigned *Bottom)
{
	for(unsigned long* Add = (unsigned long*)Top ; Add < (unsigned long*)Bottom ; Add++){
		if(inSegmentList((void*)*Add)){
			ObjHeader *Header = getObjHeader((void*)*Add);
			if(Header != NULL && Header -> Status != MARK && Header -> Status != FREE){
				Header -> Status = MARK;
				addObjHeaderToList(Header);
			}
